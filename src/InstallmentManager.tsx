...
import { getAuthMemberIdFromToken, getAuthRoleFromToken } from "./auth";
...
export default function InstallmentManager({ initialChitId }: { initialChitId: string }) {
    ...
    const memberChitId = isMemberRole && currentMemberId
        ? members.find((member) => member.id === currentMemberId)?.chit?.id ?? null
        : null;
    const visibleChits = isMemberRole && memberChitId ? chits.filter((chit) => chit.id === memberChitId) : chits;
    ...
    useEffect(() => {
        if (isMemberRole && memberChitId) {
            setChitId(memberChitId);
            if (currentMemberId) setMemberId(currentMemberId);
        }
    }, [isMemberRole, memberChitId, currentMemberId]);
    useEffect(() => {
